...
```{r}
integers <- 0 
for(i in 0:1000) {integers <- integers + i}
integers
```
...
```{r}
integers <- 0
for(i in seq(0, 1000, 2)) {integers <- integers + i}
integers
```
...
for(i in 2:ncol(survey.clean)) {
  answers <- survey.clean[,i]
  answers[(answers %in% 1:5) == FALSE] <- NA
  survey.clean[,i] <- answers
}
survey.clean
```
...
#have to subtract one "NA" when counting to account for this rows in this column

for(i.rows in 1:nrow(survey.clean)){
  na.answers <- survey.clean[i.rows,]
  num.na <- sum(is.na(na.answers)) - 1
  survey.clean$invalid.answers[i.rows] <- num.na
}
survey.clean
```